...
class OperationLogRepository:
    ...
    def find_by_idempotency_key(
        ...
        row = self.session.scalar(
            select(OperationLogRow).where(
                OperationLogRow.idempotency_key == idempotency_key
            )
        )
    ...
    def succeed(
        ...
        *,
        ...
        inverse_operation: dict[str, Any] | None,
    ) -> dict[str, Any]:
        row = self.session.get(OperationLogRow, operation_id)
        if row is None:
            raise NotFoundError(operation_id)
    ...
    def fail_running(
        self,
        error: str = "operation interrupted before commit",
    ) -> int:
        rows = self.session.scalars(
            select(OperationLogRow).where(
                OperationLogRow.status == "running"
            )
        ).all()
        if not rows:
            return 0
    ...
    def mark_reverted(
        ...
        if row.reverted_by_operation_id:
            from .repositories import ConflictError

            raise ConflictError(
                "operation has already been reverted"
            )
        row.reverted_by_operation_id = reverted_by_operation_id
        row.reverted_at = time.time()
        self.session.flush()
        return self._data(row)
    ...
    def list(
        ...
        *,
        ...
        rows = self.session.scalars(
            query.order_by(
                OperationLogRow.created_at.desc(),
                OperationLogRow.id.desc(),
            ).limit(limit)
        ).all()
