...
def run(ctx: JobContext) -> None:
    ...
    if existing is not None:
        ...
        with UnitOfWork(ctx.database) as uow:
            ...
            unit_id, project_id = job["unit_id"], job["project_id"]
        absolute = synthesize_edge(
            ctx.settings, project_id, text, voice,
            rate, pitch, ".mp3", unit_id,
        )
        ...
        asset = CommandBus(ctx.database).execute(
            PersistGeneratedFileAssetCommand(
                ...
                kind="voice", name=payload.get("name") or "配音",
                mime_type="audio/mpeg",
                generation={
                    "provider": "edge-tts", "voice": voice,
    ...
    with UnitOfWork(ctx.database) as uow:
        ...
        uow.jobs.add_event(
            ctx.job["id"], f"配音已生成 {asset['id']}",
            stage="tts", progress=0.9,
        )
