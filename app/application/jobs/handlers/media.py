...
def _asset_kind(capability: str) -> tuple[str, str]:
    return ("image/png", ".png") if capability == "image" else ("video/mp4", ".mp4")


def _complete(ctx: JobContext, asset: dict) -> None:
    with UnitOfWork(ctx.database) as uow:
        uow.jobs.update_state(
            ctx.job["id"], "running", progress=0.9,
            result={"asset_id": asset["id"]},
        )
        uow.jobs.add_event(
            ctx.job["id"], f"素材已生成 {asset['id']}",
            stage="media", progress=0.9,
        )
...
def run(ctx: JobContext) -> None:
    ...
    while status not in ("done", "failed"):
        ...
        ctx.emit(
            ...
            stage=capability, progress=progress,
        )
    ...
    asset = CommandBus(ctx.database).execute(
        PersistGeneratedAssetCommand(
            project_id=project_id, unit_id=unit_id, data=data,
            filename=f"generated{suffix}", media_store=ctx.media_store,
            kind=capability, name=payload.get("name") or f"AI {capability}",
            mime_type=mime_type,
            generation={
                "source": "provider", "provider": provider["id"],
                "prompt": prompt, "parameters": params,
                "task_id": task_id, "job_id": ctx.job["id"],
            },
        ),
        CommandContext(
